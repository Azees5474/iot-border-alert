...
const NavRow = ({
  ...
  return (
    ...
    >
      <div
        className={`p-1.5 rounded-lg ${isActive ? "bg-signal-cyan/15 text-signal-cyan" : "bg-panel-800 text-ink-500"}`}
      >
        <Icon size={17} />
      </div>
      <span className="font-medium text-sm tracking-wide">{item.name}</span>
      {isActive && (
        <div className="ml-auto w-1.5 h-1.5 bg-signal-cyan rounded-full pulse-safe" />
      )}
    </NavLink>
  );
};
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-signal-cyan/10 bg-abyss-950/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-display font-bold text-white tracking-wide leading-none">
                IOT BORDER
              </h2>
              <p className="text-[11px] text-signal-cyan font-medium tracking-[0.15em] uppercase mt-1">
                Alert System
              </p>
            </div>
          </div>
        ...
        <div className="p-4 border-t border-signal-cyan/10 bg-abyss-950/60">
          <div className="px-3 py-2.5 bg-panel-900 rounded-lg border border-signal-cyan/10 flex items-center justify-between">
            ...
            {alerts.length > 0 && (
              <span className="hud-value text-[11px] text-status-danger">
